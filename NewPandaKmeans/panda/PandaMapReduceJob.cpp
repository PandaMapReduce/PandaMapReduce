...
{	
  ...
  void PandaMapReduceJob::InitMapTasks(){
	...
	//(keyval_t *)realloc(d_g_state->h_input_keyval_arr, sizeof(keyval_t)*(len + end_row_id - start_row_id));
	for (unsigned int i= 0;i<mapTasks.size();i++){
		...
		//this->pGPUContext->input_key_vals.h_input_keyval_arr[i].keySize = chunks[i]	
		//this->pGPUContext->input_key_vals.h_input_keyval_arr[i].valSize
		//this->pGPUContext->input_key_vals.h_input_keyval_arr[i].key
		//this->pGPUContext->input_key_vals.h_input_keyval_arr[i].val
	}//for
