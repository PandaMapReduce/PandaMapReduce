...
{		
	//printf("sizeof(keyval_arr_t):%d  sizeof(keyval_t):%d  sizeof(keyval_pos_t):%d",sizeof(keyval_arr_t),sizeof(keyval_t),sizeof(keyval_pos_t));
	if (argc != 6)
	{	
		printf("usage: %s [text file][num gpu][num cpu groups][cpu/gpu work ratio][chunk size]\n", argv[0]);
		exit(-1);	
	}//if
	...
	int chunk_size = atof(argv[5]);
	if (chunk_size<100)
		chunk_size = 100;
	
	char str[256];
	//char strInput[21000];
	char *strInput = (char *)malloc(sizeof(char)*(chunk_size+1000));
	FILE *myfp;
	...
	for (int dev_id = 0; dev_id < num_gpus; dev_id++){
		//configure gpu job
		job_configuration *gpu_job_conf = CreateJobConf();//(job_configuration*)malloc(sizeof(job_configuration));

		gpu_job_conf->num_gpus = num_gpus;
		gpu_job_conf->num_mappers = 1;//num_mappers;
	...
	//char strLog[128];
	//sprintf(strLog,);
	DoLog2Disk("==JobDone== Data size:%d KB copy data:%f compute:%f cpu/gpu ratio:%f chunk_size:%d", filestatus.st_size/1024, t4-t3, t5-t4, (double)ratio,chunk_size);
